...
OMLDATASETS =  tasks$did[!(tasks$did %in% c(1476, 1459, 1486))] # can't use these tasks: no default.target provided. convertion to mlr fails.
OMLDATASET = OMLDATASETS[1] # NOTE: the line above changes the indeces of OMLDATASET

MEASURES = list(timetrain, timepredict, timeboth, mmce)

LEARNERIDS = c("rpart") # for testing (change to test other algorithms)
#LEARNERIDS = c("ranger", "rpart", "gbm", "glmnet", "naiveBayes", "nnet") # for final experiment (excluded svms since they take too long)
