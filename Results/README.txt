...
6. formulas.R: Script which produces formulas, that are used for modelling runtime. Script that is sourced by createDataframesForPred.R, since here
   the numeric predictors for the models for each target classifier are defined. The numeric predictors are needed in createDataframesForPred.R, when
   centering the numeric predictors. This script sources helperFunctions.R
...
7. helperFunctions.R: contains several heplerFunctions
   ...
   createCombinedPartialEffectPlots: creates combined partial effect plots with ggplot2, where functional estimates of the linear and 
   the smooth base-learner are summed
